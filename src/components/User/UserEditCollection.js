...
const UserEditCollection = () => {
    ...
    const editCollection = async (e) => {
        ...
        try {
            setIsLoading(true)
            if (image) {
                const response = await axiosPrivate.patch('/assets', JSON.stringify({ _id: id, name: name, image: image }))
                console.log(response.data)
                alert('🎉 update successful.')
            } else {
                const response = await axiosPrivate.patch('/assets', JSON.stringify({ _id: id, name: name }))
                console.log(response.data)
                alert('🎉 update successful.')
            }
